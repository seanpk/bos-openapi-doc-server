...
function _DocRouteFactory(app, docPubOpts) {
  ...
  function createDocEndpoint(basePath, specName, specContext) {
    var routePath = path.join(docPubOpts.docRoot, basePath),
      specPage = _renderSpec(docPubOpts.docTemplate, commonContext, specContext);
...
/**
 * Render the spec using whiskers.js and ReDoc
 *
 * @param {String} template - the whiskers template into which
 * @param {Object} commonContext - the context shared across all specs, see `_getCommonRedocContext`
 * @param {Object} specContext - the spec-specific context, such as the name, title, and spec JSON
 *
 * @returns {String} - the rendered HTML
 */
function _renderSpec(template, commonContext, specContext) {
  var context = _.merge({}, commonContext, specContext);
